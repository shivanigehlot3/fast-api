...
def add_to_cart(item: schemas.CartAdd, db: Session = Depends(get_db), current_user: User = Depends(user_required)):
...
def view_cart(db: Session = Depends(get_db), current_user: User = Depends(user_required)):
    cart_items = db.query(models.Cart).filter(models.Cart.user_id == current_user.id).all()
    return cart_items

@router.delete("/{product_id}")
def remove_from_cart(product_id: int, db: Session = Depends(get_db), current_user: User = Depends(user_required)):
    cart_item = db.query(models.Cart).filter(
        models.Cart.user_id == current_user.id,
...
def update_cart(product_id: int, item: schemas.CartUpdate, db: Session = Depends(get_db), current_user: User = Depends(user_required)):
    cart_item = db.query(models.Cart).filter(
        models.Cart.user_id == current_user.id,
        models.Cart.product_id == product_id
    ).first()
